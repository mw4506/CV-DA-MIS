...
class CombinedDataset(Dataset):
    ...
    def __getitem__(self, index):
        if index < len(self.labeled_dataset):
            ...
            # Adjust index for the unlabeled dataset
            index -= len(self.labeled_dataset)
            return self.unlabeled_dataset[index]
...
def train(args, snapshot_path):
    ...
    lesion_data_path='../data/Lesion Segmentation'
    ...
    Le_train = BraTS2019(base_dir=lesion_data_path,
                         split='train',
                         num=None,
                         transform=transforms.Compose([
                             RandomRotFlip(),
    ...
    combined_train = CombinedDataset(labeled_dataset=db_train, unlabeled_dataset=Le_train)
    # combined_train = CombinedDataset(labeled_dataset=Le_train, unlabeled_dataset=db_train)
    ...
    labeled_idxs = list(range(0, len(db_train)))
    unlabeled_idxs = list(range(len(db_train), len(combined_train)))
    # labeled_idxs = list(range(0, len(Le_train)))
    # unlabeled_idxs = list(range(len(Le_train), len(combined_train)))

    batch_sampler = TwoStreamBatchSampler(
        labeled_idxs, unlabeled_idxs, batch_size, batch_size-args.labeled_bs)

    trainloader = DataLoader(combined_train, batch_sampler=batch_sampler,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            ict_mix_factors = np.random.beta(
                args.ict_alpha, args.ict_alpha, size=(10, 1, 1, 1, 1))
            ...
            labeled_volume_batch_1 = labeled_volume_batch[0:1, ...]

            # Mix images
            batch_ux_mixed = unlabeled_volume_batch_0 * \
                (1.0 - ict_mix_factors) + \
                labeled_volume_batch_1 * ict_mix_factors
            ...
            with torch.no_grad():
                ...
                ema_output_ux1 = torch.softmax(
                    ema_model(labeled_volume_batch_1), dim=1)
